  ...
  return (
    <div className="bg-background text-foreground overflow-x-hidden">
      <section className="relative pt-32 pb-20 bg-surface overflow-hidden">
        ...
        <div className="container-page relative z-10 text-center">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            <span className="inline-block text-xs font-bold uppercase tracking-[0.25em] text-accent mb-5 px-4 py-1.5 rounded-full bg-accent/10 border border-accent/20">The People</span>
            <h1 className="text-5xl md:text-7xl font-bold tracking-tight mb-6">
              {heading.split(" ").slice(0, -1).join(" ")}{" "}
              <span className="bg-gradient-to-r from-accent via-accent/80 to-accent/60 bg-clip-text text-transparent">{heading.split(" ").slice(-1)}</span>
            </h1>
      ...
      <section className="container-page py-24">
        {members.length === 0 ? (
          <p className="text-center text-muted-foreground py-20">No team members found.</p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {members.map((member, i) => {
              const avatar = resolveAvatar(member, i);
              const initials = member.name.split(" ").map((n) => n[0]).join("").slice(0, 2);
              const avatarColors = ["from-violet-500 to-purple-600","from-blue-500 to-cyan-600","from-emerald-500 to-teal-600","from-orange-500 to-amber-600","from-pink-500 to-rose-600","from-indigo-500 to-blue-600","from-green-500 to-emerald-600","from-red-500 to-orange-600"];
              return (
                <motion.div key={member.name} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.07 }} className="group rounded-3xl border border-border bg-card overflow-hidden hover:border-accent/30 hover:shadow-2xl hover:shadow-accent/5 transition-all duration-500">
                  <div className="relative aspect-square overflow-hidden">
                    {avatar ? (
                      <img src={avatar} alt={member.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
                    ) : (
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-end p-6">
                      <div className="flex gap-3">
                        {member.linkedin && <a href={member.linkedin} target="_blank" rel="noopener noreferrer" className="size-9 rounded-full bg-white/20 backdrop-blur flex items-center justify-center text-white hover:bg-white/40 transition-colors"><Linkedin className="size-4" /></a>}
                        {member.twitter && <a href={member.twitter} target="_blank" rel="noopener noreferrer" className="size-9 rounded-full bg-white/20 backdrop-blur flex items-center justify-center text-white hover:bg-white/40 transition-colors"><Twitter className="size-4" /></a>}
                      </div>
                    </div>
                  </div>
                  <div className="p-6">
                    ...
                    {member.bio && <p className="text-muted-foreground text-sm leading-relaxed line-clamp-3">{member.bio}</p>}
                  </div>
                </motion.div>
              );
            })}
      ...
      <section className="bg-surface py-24">
        <div className="container-page">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="rounded-3xl bg-foreground text-background p-12 md:p-16 text-center relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="text-lg opacity-70 max-w-xl mx-auto mb-8">We are always looking for talented engineers, designers, and security experts who share our passion.</p>
              <a href="mailto:cifertrooper@gmail.com?subject=Job Application" className="inline-flex items-center gap-3 bg-accent text-accent-foreground px-8 py-4 rounded-full font-bold hover:bg-accent/90 transition-colors shadow-lg shadow-accent/30">Get in Touch</a>
            </div>
          </motion.div>
        </div>
      </section>
