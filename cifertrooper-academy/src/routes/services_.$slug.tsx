...
function ServiceDetailPage() {
  ...
  // Try backend first; fall back to SERVICES_DATA
  ...
  const staticData = SERVICES_DATA[slug] ?? null;

  // Build a unified ServiceDetail from backend or static
  const data: ServiceDetail | null = apiService
    ? {
        title: apiService.title,
        subtitle: apiService.description,
        heroImageUrl: apiService.image,
        // Backend doesn't have rich detail yet — merge with static if available
        ...(staticData ?? {}),
        title: apiService.title,
        subtitle: apiService.description,
      }
  ...
  if (isLoading) return <div className="py-20 text-center text-muted-foreground">Loading service details...</div>;
  if (!data) return <div className="py-20 text-center text-muted-foreground">Service not found.</div>;
