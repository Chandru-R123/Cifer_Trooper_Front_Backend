...
export function SocialShareBar({ url, title }: { url: string; title: string }) {
  ...
  const links = [
    { Icon: Twitter, href: `https://twitter.com/intent/tweet?url=${enc(url)}&text=${enc(title)}`, label: "Share on Twitter" },
    { Icon: Linkedin, href: `https://www.linkedin.com/sharing/share-offsite/?url=${enc(url)}`, label: "Share on LinkedIn" },
    { Icon: Facebook, href: `https://www.facebook.com/sharer/sharer.php?u=${enc(url)}`, label: "Share on Facebook" },
  ];
  return (
